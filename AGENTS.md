...
- **Portaal**: `/portaal` is de zelfbeheer-omgeving voor verkopers met een gekoppeld account (`requireSellerUser()` in authz.ts). Koppelen doet het team in het beheer (of automatisch bij goedkeuring op e-mailmatch); zelf claimen met mailverificatie komt later. Bewerken kan alleen de eigen vermelding en alleen de subset telefoon/website/omschrijving/uren/producten via `updateOwnProducerAction` (hergebruikt `AdminProducerForm` met `editableFields` + eigen action-prop); naam, adres en status blijven bij het team.
