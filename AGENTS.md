...
- **Cheffs** (lijst-chat, naam van Frank met bewuste dubbel-f; enkelvoud cheff, meervoud cheffs): chat per lijst tussen echte mensen, geen AI. `list_messages` (user_id verplicht: schrijven alleen ingelogd; lezen voor iedereen met de lijst-link, zelfde vertrouwensmodel als de lijst). Algemene chat = vrij invoerveld; **item-vragen zijn alleen presets** (aantikken = versturen, koksmuts-knopje op de itemrij, chip "Over: {item}", `item_label` overleeft item-verwijdering). Ongelezen-teller device-lokaal (`of_chefs_seen:<token>`); realtime via het bestaande bump/refresh-kanaal. Ook op elke producentpagina: "Vraag je cheffs" (`AskChefsButton`, alleen met actieve lijst) stuurt een preset met klikbare producent-chip (`producer_slug/producer_name`, naam altijd uit de DB) naar de chat van je actieve lijst.
