...
- **Portaal**: `/portaal` is de zelfbeheer-omgeving voor verkopers met een gekoppeld account (`requireSellerUser()` in authz.ts): bedrijfsprofiel met tabs Overzicht/Gegevens/Foto's/Producten. Koppelen doet het team in het beheer (of automatisch bij goedkeuring op e-mailmatch); zelf claimen met mailverificatie komt later. Bewerken kan alleen de eigen vermelding en alleen de subset telefoon/website/omschrijving/uren/producten (hergebruikt `AdminProducerForm` met `editableFields` + eigen action-prop); naam, adres en status blijven bij het team. Producten = de `offers`-tabel (prikbord-model, prijsindicatie als tekst, geen betalingen); publieke pagina toont galerij + Aanbod.
...
- **Swipe-deck**: 4e nav-tab "Swipen" → `/lijst/[token]/swipen` (`SwipeDeck.tsx`, pointer events + dominante-as-check, `of-fly-*`-keyframes; knoppen-fallback verplicht). Deck-volgorde: `bought_stats` (accumulerende koophistorie, upsert in `setItemChecked`, huishouden-scope waar mogelijk, wisbestendig) > `itemsInSeason` > `BASICS` (gecureerde basisset in catalog.ts); open items uitgefilterd. Tegelwand toont 6 tegels per categorie + "Toon alles".
