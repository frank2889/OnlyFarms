...
- **Accountfilosofie**: één account per persoon, petten bepalen de toegang (rol "gebruiker"/"team", straks verkoper via `sellers.user_id`); een boer is zelf ook klant. Team- en ledenomgeving blijven bewust gescheiden (harde veiligheidsgrens, per doelgroep één URL); eventueel later één slimme voordeur die op rol doorstuurt.
