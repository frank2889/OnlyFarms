...
- **Cart-drawer-model**: de lijstpagina is een "winkel" (tegelwand centraal); je lijst is een drawer die je opent met de Lijst-tab. Toevoegen via de vaste balk onderin, die een zoek-overlay opent met het veld bovenaan (toetsenbord zit nooit in de weg). Tegels werken als toggle (Bring-patroon): tik = op de lijst, nog een tik = er weer af; vasthouden = aantal kiezen. Afvinken toont eerst een vinkje, dan schuift het item naar gekocht. Lijsten zijn te hernoemen en verwijderen via het lijstmenu. Onderste panelen eindigen altijd boven de tabbalk.
...
- **Catalogus**: 160 items, incl. noten (walnoot, amandel, cashewnoot, pinda) en zaden (zonnebloempit, pompoenpit, lijnzaad, chiazaad, sesamzaad) onder "Overig".
