**Update 3 aug 2026**: de twee kleinste, veiligste aanbevelingen zijn doorgevoerd. Openingstijden-parser uitgebreid met een `dagelijks`-zonder-eigen-tijdvak-detectie (bewust met een vangnet tegen dubbelepunt-loze tijden zoals "14-18", zodat samengestelde schema's niet per ongeluk als "de hele dag open" verschijnen): +16 records nu parseerbaar (1693 → 1709 van 1931). `kind` afgeleid uit de bedrijfsnaam via `scripts/classify-kind.ts` (idempotent, alleen naam-gebaseerd en met dry-run gecontroleerd): 31 imkerij, 7 wijngaard, 3 brouwerij omgezet (41 van de 2.280). Bakkerij bewust overgeslagen: "bakker" is een veelvoorkomende achternaam (bijv. "Bakker's Farm"), te onbetrouwbaar signaal zonder handmatige beoordeling. Naamduplicaten-check toegevoegd aan `scripts/data-quality.ts` (stap 4, non-destructief). Duplicaten opruimen en de Google-sync blijven open staan, zie onder.
