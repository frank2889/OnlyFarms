...
- **Delen is nog minimaal**: "Deel deze lijst" doet alleen de OS-share-sheet of link kopiëren, er is nergens zichtbaar met wie je een lijst deelt of wie er al is toegevoegd. Frank opperde ook een in-app chat binnen de lijst: **chatten tussen de echte mensen op een gedeelde lijst** (ingelogde gezinsleden/huisgenoten onderling, nadrukkelijk géén AI-chat). Richting nog niet gekozen; eerste stap is vermoedelijk een "wie doet mee"-overzicht (leunt op de bestaande household_members-data), de chat zelf is een apart, groter project (berichten-tabel, ongelezen-badges, realtime via Pusher, moderatie).
